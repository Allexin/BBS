...
- изменение критичных counters за 24 часа и 30 дней;
...
- новый critical-condition или увеличение critical counter — немедленный alert;
- неизменившийся critical не повторяется немедленно, но присутствует в каждом
  суточном heartbeat;
- warning-condition и рост warning counter отдельного alert не создают и входят в
  ближайший суточный heartbeat;
...
Дедупликация немедленного critical выполняется по configured disk ID, rule ID и
наблюдаемому значению/переходу, а не по номеру executor run.
