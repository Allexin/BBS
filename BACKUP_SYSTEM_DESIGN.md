...
Первоначально все операции выполняются последовательно. Каждая операция manager
соответствует одному запуску executor. Управление состоянием backup-диска является
опциональным свойством job: при наличии блока `disk` executor переводит этот диск
online и гарантированно возвращает offline; без блока `disk` репозиторий считается
постоянно доступным и executor не меняет состояние дисков или mount points.
Параллельное выполнение не входит в v1.
...
Каждый `failed` или `interrupted` backup обязан быть наблюдаемым одновременно через
job/system health в Web UI, немедленное Telegram-уведомление и приватный diagnostic
log с первичным типом и цепочкой причины. Публичная JSON-проекция не содержит пути и
текст исключения. `INTERNAL_ERROR` без сохранённой приватной причины считается
отдельным дефектом observability и не является допустимым штатным результатом.
...
Блок `disk` в snapshot, mirror и maintenance job необязателен. Его отсутствие означает
режим `always-online`: executor проверяет marker по настроенному абсолютному пути,
выполняет SMART preflight и операцию под общей блокировкой, но не вызывает disk-control.
Этот режим предназначен для постоянно подключённых локальных хранилищ. Наличие блока
`disk` включает `managed-offline` lifecycle для отдельного защитного носителя. Режимы
задаются конфигурацией конкретной job, а не свойствами буквы диска или всего сервиса.
Disk safety latch создаётся только для job с настроенным `managed-offline` блоком
...
Тот же факт публикуется как нейтральное информационное поле карточки job в Web UI;
он не меняет health, цвет карточки или результат запусков.
...
При первом появлении валидированной snapshot job manager создаёт её scheduler state
с `slot_counter = 0` и без recovery flag. Это отличается от потери состояния уже
известной job, которая по-прежнему требует full recovery-check. Первый backup новой
job выполняет bounded provisioning локального restic-репозитория: `restic init`
разрешён только когда repository path отсутствует либо является пустым каталогом.
Наличие любых файлов без restic `config` считается конфликтом и останавливает job;
существующий репозиторий никогда не переинициализируется.

Executor terminal protocol сохраняет внутренний disk lifecycle outcome и для jobs,
которым перевод offline не требуется. Однако journal/Web UI не публикуют
`disk_offline_confirmed` для always-online job без секции `disk`: фактической операции
offline там не было. Для job с управляемым носителем это событие публикуется как
подтверждение реально выполненного возврата диска offline.
...
- список выполненных backup jobs, агрегированный до одной строки на job: число
  запусков по terminal result, результат и длительность последнего запуска;
- компактный список ошибок не-backup operations, агрегированный до одной строки на
  сочетание job + operation kind;
...
Срочный alert не исключает событие из следующего суточного отчёта, но failed backup
не дублируется между секциями backup и errors. Повторные runs одной job сворачиваются
в счётчики; повторные сообщения одного и того же alert дедуплицируются по типу
события и run.
...
Если локальное cron-время не существует при весеннем DST-переходе, слот пропускается
без catch-up и без продвижения cycle. Если локальное время неоднозначно при осеннем
переходе, оно срабатывает ровно один раз в первом (`fold=0`) вхождении; повторный
локальный момент не создаёт вторую operation.
...
Режим `keep-all` задаётся без числовых tiers:
...
В этом режиме backup-run не вызывает `forget`: все snapshots сохраняются до отдельного
решения оператора. Это предназначено для малых архивных наборов, где автоматическое
удаление запрещено. `snapshots` всё равно перечитывается после backup и публикуется в
результате. Наличие `keep_*` рядом с `mode: keep-all` является config error.
...
В режиме `policy` retention является обязательной частью snapshot-backup. Если новый snapshot создан,
...
Для каждого диска executor независимо от результата публикует privacy-safe итог
self-test: необратимый identity key, тип теста, `success|failed|timeout|unsupported`,
классифицированную причину, время начала/окончания и последнюю доступную величину
оставшегося процента. Manager связывает итог с run и хранит его отдельно от пассивных
SMART-наблюдений. После попытки self-test executor в любом случае пытается снять
свежие пассивные SMART-показатели каждого диска discovery snapshot; ошибка одного
такого чтения также сохраняется и не отменяет уже полученные результаты остальных.
Таким образом failed all-system run не оставляет UI на старом успешном снимке.
...
Первое успешное наблюдение по-прежнему не создаёт trend regression, но впервые
обнаруженное абсолютное critical-condition создаёт отдельный немедленный alert.
Повтор неизменного condition не дублируется; новое condition либо новое увеличение
counter создаёт новый alert.
...
Пустые DOM-контейнеры не получают placeholder-текст. Любое отсутствующее отображаемое
значение выводится только рядом с явной меткой поля (`Manufacturer: —`, `Capacity: —`),
чтобы несколько неизвестных свойств нельзя было спутать между собой.
Карточки сортируются сначала по effective severity (`critical`, `warning`, `unknown`,
`healthy`), затем по display identity. Каждая карточка публикует конкретные health
reasons (например, `4 pending sectors`), а не только общий цвет. Если последнее
наблюдение старше `smart.yaml:stale_after_hours`, passive health становится `unknown`
с причиной staleness; старые metrics остаются видимыми вместе с timestamp, но не
выдаются за актуальное состояние.
