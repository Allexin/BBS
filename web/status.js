...
const formatDuration = seconds => { if(seconds==null)return "not recorded"; const value=Math.max(0,Math.floor(seconds));if(value<60)return `${value}s`;if(value<3600)return `${Math.floor(value/60)}m ${value%60}s`;return `${Math.floor(value/3600)}h ${Math.floor(value%3600/60)}m`; };
...
const addFactLink = (list,label,value,href) => { const detail=document.createElement("dd"),link=text("a",value);link.href=href;link.target="_blank";link.rel="noopener";detail.append(link);list.append(text("dt",label),detail); };
...
function renderJobs(items,operations){const active=new Map(operations.map(item=>[item.job_id,item]));const root=byId("jobs");root.replaceChildren();items.forEach(job=>{const card=text("article",null,"card");const head=document.createElement("header");const title=document.createElement("div");title.append(text("h3",job.display_name),text("p",`${job.kind} · ${job.job_id}`,"muted"));head.append(title,text("span",job.health,"badge "+job.health));const facts=text("dl",null,"facts");const current=active.get(job.job_id);if(current)addFact(facts,"Current status",operationStatus(current));addFact(facts,"Reason",job.health_reason);if(job.source_read_error_count){addFact(facts,"Read errors",`${job.source_read_error_count} file(s)`);job.source_read_error_paths.forEach((path,index)=>addFact(facts,`Unreadable ${index+1}`,path));if(job.source_read_error_count>job.source_read_error_paths.length)addFact(facts,"More unreadable",`${job.source_read_error_count-job.source_read_error_paths.length} more file(s)`);if(job.source_read_error_report)addFactLink(facts,"Full report","View full list",job.source_read_error_report);}if(job.protection_info)addFact(facts,"Info",job.protection_info);addFact(facts,"Last run",runLabel(job.last_run));addFact(facts,"Last duration",formatDuration(job.last_completed_duration_seconds));addFact(facts,"Previous",runLabel(job.previous_run));addFact(facts,"Last success",formatTime(job.last_success_at));addFact(facts,"Next",`${job.next_operation??"unknown"} · ${formatTime(job.next_fire_at)}`);addFact(facts,"Deadline",job.deadline??"not configured");addFact(facts,"Repository size",job.backup_metrics?.repository_physical_bytes==null?"not reported":formatBytes(job.backup_metrics.repository_physical_bytes));addFact(facts,"Repository measured",job.backup_metrics?.observed_at?formatTime(job.backup_metrics.observed_at):"not reported");addFact(facts,"Protected",formatBytes(job.backup_metrics?.protected_logical_bytes));addFact(facts,"Added",formatBytes(job.backup_metrics?.repository_added_bytes));card.append(head,facts);root.append(card);});}
...
function renderDisks(items){
  ...
  items.forEach(disk=>{
    const card=text("details",null,"card disk-card"),overview=document.createElement("summary");
    ...
    if(disk.health_reasons.length)overview.append(text("p",disk.health_reasons.join(" · "),disk.smart_health==="critical"?"health-reasons critical-box":"health-reasons"));
...
async function loadStatus(){try{const health=await fetch("/backup-status/health.json",{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error(`health HTTP ${r.status}`);return r.json();});const data=await fetch("/backup-status/status.json",{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error(`status HTTP ${r.status}`);return r.json();});if(data.generation_id!==health.generation_id)throw new Error("projection generation is changing; retrying");const projectionAge=(Date.now()-new Date(data.generated_at).getTime())/1000;if(!Number.isFinite(projectionAge)||projectionAge>health.status_stale_after_seconds)throw new Error(`BBS service unavailable: status heartbeat is stale (${Math.max(0,Math.floor(projectionAge))}s old)`);renderHealth(data,health);renderOperations(data.operations);renderJobs(data.jobs,data.operations);renderDisks(data.disks);renderVolumes(data.volumes);byId("updated").textContent=`updated ${formatTime(data.generated_at)}`;byId("connection").className="notice hidden";}catch(error){byId("connection").textContent=String(error);byId("connection").className="notice error";}}
