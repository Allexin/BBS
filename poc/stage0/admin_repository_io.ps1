...
function Invoke-DiskPart {
    param([string[]]$Commands, [switch]$AllowAlreadyDetached)
    $Commands | Set-Content -LiteralPath $script:DiskPartScript -Encoding ASCII
    $output = & diskpart.exe /s $script:DiskPartScript 2>&1
    $text = $output -join "`n"
    if (-not $AllowAlreadyDetached -and ($LASTEXITCODE -ne 0 -or $text -match 'DiskPart has encountered an error')) {
        throw "diskpart failed:`n$text"
    }
}
...
$fixtureRoot = 'C:\BBSStage0RepositoryIo'
$vhdPath = Join-Path $fixtureRoot 'repository-io.vhdx'
...
if ([IO.Path]::GetFullPath($fixtureRoot).TrimEnd('\') -ne 'C:\BBSStage0RepositoryIo') {
...
try {
    Invoke-DiskPart @(
        "create vdisk file=`"$vhdPath`" maximum=768 type=expandable",
        "select vdisk file=`"$vhdPath`"",
        'attach vdisk',
        'create partition primary',
        'format fs=ntfs quick label=BBS_STAGE0_IO',
        'assign letter=R'
    )
